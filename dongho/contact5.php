...
if (!isset($_SESSION['user_id'])) {
    ...
    $userId = $_SESSION['user_id']; // Lấy ID của người dùng từ session
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($name) || empty($email) || empty($phone) || empty($subject) || empty($content)) {
            ...
            // Lấy thông tin của người dùng hiện tại từ CSDL
            $query = $conn->prepare("SELECT * FROM users WHERE id = ?");
            $query->execute([$userId]);
            $user = $query->fetch(PDO::FETCH_ASSOC);

            // Kiểm tra nếu email và số điện thoại khớp với thông tin của người dùng hiện tại
