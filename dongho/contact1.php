...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Tự động điền số điện thoại dựa trên email
    if (isset($_POST['email_check'])) {
        ...
        echo json_encode(['phone' => $user['phone'] ?? '']);
        exit;
    }

    // Đảm bảo token CSRF hợp lệ
    ...
    if (!$user_id) {
        ...
        if (empty($name) || empty($email) || empty($phone) || empty($subject) || empty($content)) {
            ...
            try {
                ...
                // Kiểm tra thông tin email và số điện thoại khớp với tài khoản
                if ($user && $user['email'] == $email && $user['phone'] == $phone) {
                    ...
                    $checkContentQuery->execute([$user_id, $content]);
                    ...
                    if ($existingMessage) {
                        ...
                        $insertQuery->execute([$user_id, $name, $email, $phone, $subject, $content]);
...
<main>
    <div class="contact-container">
        ...
        <?php if ($message): ?>
            <p class="notification <?php echo ($message == "Liên hệ của bạn đã được gửi thành công!") ? 'success' : ''; ?>">
                <?php echo $message; ?>
            </p>
        <?php endif; ?>
        ...
        <form action="contact.php" method="POST" class="contact-form">
            ...
            <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($_POST['email'] ?? '', ENT_QUOTES); ?>" required onblur="fetchPhoneNumber(this.value)">

            <label for="phone">Số điện thoại:</label>
            <input type="tel" id="phone" name="phone" value="<?php echo htmlspecialchars($_POST['phone'] ?? '', ENT_QUOTES); ?>" required>

            <label for="subject">Chủ đề:</label>
            <input type="text" id="subject" name="subject" value="<?php echo htmlspecialchars($_POST['subject'] ?? '', ENT_QUOTES); ?>" required>

            <label for="content">Nội dung:</label>
            <textarea id="content" name="content" required><?php echo htmlspecialchars($_POST['content'] ?? '', ENT_QUOTES); ?></textarea>

            <button type="submit" class="contact-btn">Gửi liên hệ</button>
        </form>
    </div>
</main>
<script>
    // JavaScript: Fetch phone number based on email
    function fetchPhoneNumber(email) {
        if (email) {
            fetch('contact.php', {
                method: 'POST',
                headers: {
                ...
                body: new URLSearchParams({ email_check: email })
            })
