...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password !== $confirm_password) {
        ...
        $message = "Tên tài khoản phải từ 5 đến 20 ký tự.";
    } elseif (strlen($password) < 6) {
        $message = "Mật khẩu phải ít nhất 6 ký tự.";
    } else {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT); // Mã hóa mật khẩu

        // Kiểm tra trùng tên tài khoản, email hoặc số điện thoại
        $checkQuery = $conn->prepare("SELECT * FROM users WHERE username = ? OR email = ? OR phone = ?");
        $checkQuery->execute([$username, $email, $phone]);

        if ($checkQuery->rowCount() > 0) {
            $message = "Tên tài khoản, email hoặc số điện thoại đã tồn tại!";
        } else {
            // Thêm người dùng vào cơ sở dữ liệu
            $query = $conn->prepare("INSERT INTO users (username, password, email, phone, address) VALUES (?, ?, ?, ?, ?)");
            $query->execute([$username, $hashed_password, $email, $phone, $address]);
            ...
            // Đặt thông báo đăng ký thành công và chuyển hướng
...
<main>
    <div class="register-container">
        ...
        <!-- Hiển thị thông báo -->
        <?php if ($message): ?>
            <p class="notification <?php echo ($message == "Đăng ký thành công!") ? 'success' : ''; ?>">
                <?php echo $message; ?>
            </p>
        <?php endif; ?>
