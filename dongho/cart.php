...
// Check if the user is logged in before proceeding with the order
if (isset($_POST['order'])) {
    // If user is not logged in, show an error message and redirect to login
    if (!$user_id) {
        $error_message = "Bạn cần đăng nhập để đặt hàng.";  // Message asking the user to log in
    } else {
...
foreach ($cartItems as $item) {
    ...
    $productName = $stmtProduct->fetchColumn(); // Lấy tên sản phẩm

    // Truy vấn tên khách hàng
    $stmtUser = $conn->prepare("SELECT username FROM users WHERE id = ?");
    $stmtUser->execute([$user_id]);
    $customerName = $stmtUser->fetchColumn(); // Lấy tên khách hàng
    ...
    $orderItems[] = [
        ...
        'ten_sanpham' => $productName, // Gán tên sản phẩm
        'ten_khachhang' => $customerName // Gán tên khách hàng
    ];
    ...
    $updateProductStmt->execute([$newQuantity, $item['product_id']]);
        ...
        foreach ($orderItems as $item) {
         $stmt = $conn->prepare("
    INSERT INTO nguoinhan_product (order_id, product_id, ten_sanpham, ten_khachhang, code_product, soluong) 
    VALUES (?, ?, ?, ?, ?, ?)
");

$stmt->execute([
    $orderId,                 // Đảm bảo $orderId đã được định nghĩa
    $item['product_id'],      // Kiểm tra biến này tồn tại
    $item['ten_sanpham'],     // Đảm bảo có giá trị
    $item['ten_khachhang'],   // Đảm bảo có giá trị
    $item['code_product'],    // Kiểm tra biến này tồn tại
    $item['soluong']          // Kiểm tra biến này tồn tại
]);
        }
...
// If there's an error message (e.g., not logged in), display it to the user
...
<div class="risk-free-shopping">
    ...
    <div class="risk-free-content">
        <div class="risk-box">
            ...
            <p>Miễn phí vận chuyển các đơn nội thành<br>Ngoại thành từ 500K</p>
        </div>
        <div class="risk-box">
