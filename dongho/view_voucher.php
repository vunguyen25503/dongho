...
// Query to get data from the discount_codes table
$sql = "SELECT id, code, discount_percent, description, start_date, end_date, status FROM discount_codes";
            ...
            if ($result->rowCount() > 0) {
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr>
                        ...
                        <td>
                            <a href='edit_voucher.php?id=" . $row["id"] . "' class='btn btn-warning btn-sm'>Sửa</a>
                            <a href='delete_voucher.php?id=" . $row["id"] . "' class='btn btn-danger btn-sm' onclick=\"return confirm('Bạn có chắc chắn muốn xóa mã giảm giá này?');\">Xóa</a>
                        </td>
                    </tr>";
                }
            } else {
